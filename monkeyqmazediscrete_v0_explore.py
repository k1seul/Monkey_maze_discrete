...
class MonkeyQmazeDiscreteV0Ex(gym.Env):
    """
    ...
    def init_map(self): 
        ...
        self.small_reward = self.road_map
    ...
    def _get_info(self):
        ...
        distance_vec = self.trial_goal - self._agent_location

        info = np.linalg.norm(distance_vec)

        info_reward = -0.5*float(info) + 1.5
    ...
    def step(self, action):
        ...
        terminated = True if self.trial_small_reward.size == 0 else False 
        ...
        if terminated:
            ...
            reward = 1*(self.max_episode_step - self.step_count)/self.max_episode_step
        elif wall_hit:
            reward = -0.1
        else:
            reward = 0
